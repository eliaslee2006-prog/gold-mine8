...
const PB_URL='/api/v8/phase9d2/pinboard';
const AUDIO_URL='/api/v8/phase9d2/task-audio';
const PRESET_BG=['#09090d','#111217','#171321','#211216','#2b1819','#3b2522','#e7dfd2'];
const PRESET_FILL=['#573b42','#8b263b','#30191f','#101116','#d7c6b8','#b17f8a','#65755d','#2d4964'];
let notify=()=>{};

function toast(msg,error=false){
  const t=$('#toast');
  if(t){t.textContent=(error?'ERROR // ':'')+msg;t.classList.add('show');clearTimeout(toast._t);toast._t=setTimeout(()=>t.classList.remove('show'),3200);}
  else console[error?'error':'log'](msg);
}
function fmtMoney(v,currency='SGD'){try{return new Intl.NumberFormat('en-SG',{style:'currency',currency,maximumFractionDigits:2}).format(Number(v)||0)}catch{return `${currency} ${(Number(v)||0).toFixed(2)}`}}
...
function migrateBoard(d){
  const raw=d?.board&&typeof d.board==='object'?d.board:{};
  if(Array.isArray(raw.pages)&&raw.pages.length){return normalizeBoard(raw,d.background);}
  const p={id:uid('page'),name:'OPS // 01',background:d?.background||'#171321',items:Array.isArray(raw.items)?raw.items:[],strokes:Array.isArray(raw.strokes)?raw.strokes:[]};
  return normalizeBoard({version:3,currentPageId:p.id,pages:[p],shelves:Array.isArray(raw.shelves)?raw.shelves:[]},d?.background);
}
function normalizeBoard(b,bg='#171321'){
  const pages=(b.pages||[]).map((p,pi)=>({id:p.id||uid('page'),name:p.name||`BOARD ${String(pi+1).padStart(2,'0')}`,background:p.background||bg||'#171321',items:(p.items||[]).map((i,ii)=>normalizeItem(i,ii)),strokes:Array.isArray(p.strokes)?p.strokes:[]}));
  if(!pages.length)pages.push({id:uid('page'),name:'OPS // 01',background:bg||'#171321',items:[],strokes:[]});
  return{version:3,currentPageId:pages.some(p=>p.id===b.currentPageId)?b.currentPageId:pages[0].id,pages,shelves:Array.isArray(b.shelves)?b.shelves:[]};
}
function normalizeItem(i,n=0){return{...i,id:i.id||uid('pin'),x:Number.isFinite(Number(i.x))?Number(i.x):30+n*18,y:Number.isFinite(Number(i.y))?Number(i.y):30+n*18,w:clamp(i.w||220,110,1000),h:clamp(i.h||140,70,900),z:Number(i.z)||n+1,locked:!!i.locked,hidden:!!i.hidden,style:{fill:i.style?.fill||defaultFill(i.type),text:i.style?.text||'#fff4ee',border:i.style?.border||'#ff315c',opacity:i.style?.opacity==null?.94:clamp(i.style.opacity,.1,1),radius:i.style?.radius??(i.type==='shape'?34:8),fontSize:i.style?.fontSize||15,glow:i.style?.glow??18,variant:i.style?.variant||'field'}}}
function defaultFill(type){return type==='text'?'#101116':type==='shape'?'#6d1930':type==='file'?'#101116':'#573b42'}
function activePage(){return boardState?.pages?.find(p=>p.id===boardState.currentPageId)||boardState?.pages?.[0]||null;}
async function saveBoard(){clearTimeout(saveTimer);try{const p=activePage();await api(PB_URL,{method:'PATCH',body:{background:p?.background||'#171321',board:boardState}});}catch(e){console.warn('9D.3 pinboard save',e);toast(`PINBOARD SAVE // ${e.message}`,true)}}
function queueBoardSave(){clearTimeout(saveTimer);saveTimer=setTimeout(saveBoard,400)}
...
function bindItem(el,i){
  ...
  const txt=el.querySelector('.p9d3-item-text');if(txt)txt.oninput=()=>{i.text=txt.textContent||'';queueBoardSave()};
  const grip=el.querySelector('.p9d3-item-grip');grip.onpointerdown=e=>startItemDrag(e,i,el);
  el.addEventListener('pointerdown',e=>{if(e.pointerType==='mouse'||e.target.closest('.p9d3-item-grip,.p9d3-resize,[contenteditable],a'))return;clearTimeout(longPressTimer);longPressTimer=setTimeout(()=>{selectedId=i.id;renderSelection();openInspector(i,e.clientX,e.clientY);navigator.vibrate?.(15)},520);},{passive:true});
  el.addEventListener('pointerup',()=>clearTimeout(longPressTimer),{passive:true});el.addEventListener('pointercancel',()=>clearTimeout(longPressTimer),{passive:true});el.addEventListener('pointermove',()=>clearTimeout(longPressTimer),{passive:true});
  const rh=el.querySelector('.p9d3-resize');rh.onpointerdown=e=>startResize(e,i,el);
}
function startItemDrag(e,i,el){if(i.locked||drawMode)return;e.preventDefault();selectedId=i.id;i.z=Math.max(...activePage().items.map(x=>Number(x.z)||1),1)+1;const b=$('#p9d3Board'),r=b.getBoundingClientRect(),sx=e.clientX,sy=e.clientY,ox=i.x,oy=i.y;el.setPointerCapture?.(e.pointerId);const move=ev=>{const nx=ox+ev.clientX-sx,ny=oy+ev.clientY-sy;i.x=clamp(nx,0,Math.max(0,r.width-i.w));i.y=clamp(ny,0,Math.max(0,r.height-i.h));el.style.left=`${i.x}px`;el.style.top=`${i.y}px`;};const end=()=>{el.removeEventListener('pointermove',move);el.removeEventListener('pointerup',end);el.removeEventListener('pointercancel',end);queueBoardSave();renderLayers();};el.addEventListener('pointermove',move);el.addEventListener('pointerup',end);el.addEventListener('pointercancel',end);}
...
function addPage(){const p={id:uid('page'),name:`OPS // ${String(boardState.pages.length+1).padStart(2,'0')}`,background:activePage()?.background||'#171321',items:[],strokes:[]};boardState.pages.push(p);boardState.currentPageId=p.id;selectedId=null;renderBoard();queueBoardSave();}
function deletePage(){if(boardState.pages.length<=1)return toast('PINBOARD REQUIRES AT LEAST ONE PAGE',true);const p=activePage();if(!confirm(`Delete ${p.name}?`))return;const i=boardState.pages.indexOf(p);boardState.pages.splice(i,1);boardState.currentPageId=boardState.pages[Math.max(0,i-1)].id;selectedId=null;renderBoard();queueBoardSave();}
function canvasSetup(){const c=$('#p9d3Canvas'),b=$('#p9d3Board');if(!c||!b)return null;const dpr=Math.min(2,devicePixelRatio||1),w=Math.max(1,b.clientWidth),h=Math.max(1,b.clientHeight);if(c.width!==Math.round(w*dpr)||c.height!==Math.round(h*dpr)){c.width=Math.round(w*dpr);c.height=Math.round(h*dpr);c.style.width=`${w}px`;c.style.height=`${h}px`;c.dataset.dpr=dpr;}const ctx=c.getContext('2d');ctx.setTransform(dpr,0,0,dpr,0,0);return{c,b,ctx,dpr,w,h};}
function renderDrawing(){const setup=canvasSetup(),p=activePage();if(!setup||!p)return;const {ctx,w,h}=setup;ctx.clearRect(0,0,w,h);for(const s of p.strokes||[]){if(!s.points?.length)continue;ctx.save();ctx.globalAlpha=clamp(s.alpha??1,.05,1);ctx.lineCap='round';ctx.lineJoin='round';ctx.lineWidth=s.width||4;ctx.strokeStyle=s.color||'#ff315c';if(s.tool==='eraser')ctx.globalCompositeOperation='destination-out';else if(s.tool==='smudge'){ctx.globalCompositeOperation='source-over';ctx.globalAlpha*=.22;ctx.filter='blur(5px)';ctx.lineWidth=(s.width||4)*2.2;}ctx.beginPath();ctx.moveTo(...s.points[0]);for(const q of s.points.slice(1))ctx.lineTo(...q);ctx.stroke();ctx.restore();}}
function drawStart(e){if(!drawMode)return;const c=$('#p9d3Canvas'),r=c.getBoundingClientRect(),p=activePage(),s={id:uid('stroke'),tool:drawTool,color:drawColor,width:drawTool==='brush'?drawWidth*1.8:drawTool==='smudge'?drawWidth*2.4:drawWidth,alpha:drawAlpha,points:[[e.clientX-r.left,e.clientY-r.top]]};p.strokes.push(s);c.setPointerCapture?.(e.pointerId);const move=ev=>{s.points.push([ev.clientX-r.left,ev.clientY-r.top]);renderDrawing();};const end=()=>{c.removeEventListener('pointermove',move);c.removeEventListener('pointerup',end);c.removeEventListener('pointercancel',end);queueBoardSave();};c.addEventListener('pointermove',move);c.addEventListener('pointerup',end);c.addEventListener('pointercancel',end);}
function setDraw(on){drawMode=!!on;$('#p9d3DrawTools').classList.toggle('hidden',!on);$('#p9d3Board').classList.toggle('drawing',on);$('#p9d3Draw').classList.toggle('active',on);}
function bindPinboard(){
  $$('[data-add]').forEach(b=>b.onclick=()=>addItem(b.dataset.add));$('#p9d3Draw').onclick=()=>setDraw(!drawMode);$('#p9d3DoneDraw').onclick=()=>setDraw(false);$('#p9d3Canvas').onpointerdown=drawStart;$('#p9d3BrushSize').oninput=e=>drawWidth=Number(e.target.value)||4;$('#p9d3BrushOpacity').oninput=e=>drawAlpha=Number(e.target.value)/100;$('#p9d3BrushColor').oninput=e=>drawColor=e.target.value;$$('[data-draw]').forEach(b=>b.onclick=()=>{drawTool=b.dataset.draw;$$('[data-draw]').forEach(x=>x.classList.toggle('active',x===b));});$('#p9d3UndoStroke').onclick=()=>{activePage().strokes.pop();renderDrawing();queueBoardSave()};
  $('#p9d3Upload').onclick=()=>$('#p9d3UploadInput').click();$('#p9d3UploadInput').onchange=e=>{const f=e.target.files?.[0];e.target.value='';if(f)uploadPin(f)};$('#p9d3Delete').onclick=()=>{const i=selectedItem();if(i)inspectorAction(i,'delete')};
  $('#p9d3PrevPage').onclick=()=>changePage(-1);$('#p9d3NextPage').onclick=()=>changePage(1);$('#p9d3AddPage').onclick=addPage;$('#p9d3PageCounter').ondblclick=deletePage;
  $('#p9d3LayersBtn').onclick=()=>$('#p9d3Layers').classList.toggle('hidden');$('#p9d3CloseLayers').onclick=()=>$('#p9d3Layers').classList.add('hidden');$('#p9d3BgBtn').onclick=()=>$('#p9d3BgPalette').classList.toggle('hidden');$('[data-close-bg]').onclick=()=>$('#p9d3BgPalette').classList.add('hidden');$$('[data-bg]').forEach(b=>b.onclick=()=>{activePage().background=b.dataset.bg;renderBoard();queueBoardSave()});$('#p9d3BgCustom').oninput=e=>{activePage().background=e.target.value;$('#p9d3Board').style.background=e.target.value;queueBoardSave()};
  window.addEventListener('resize',()=>{renderBoard();},{passive:true});
}

// ---------------- CASSETTE V3 ----------------
let audios=[],audioMap=new Map(),audioBlobUrl=null,cassette=null,raf=0,lastFrame=0,reelAngle=0,reverseCtx=null,reverseBuffer=null,reverseSource=null,reverseGain=null,reverseStartCtx=0,reverseStartPos=0;
function cassettePrefs(id){try{return JSON.parse(localStorage.getItem(`neon9d3:cassette:${id}`)||'{}')}catch{return{}}}
function saveCassettePrefs(id,p){localStorage.setItem(`neon9d3:cassette:${id}`,JSON.stringify(p));}
async function loadAudios(){try{const d=await api(AUDIO_URL);audios=d.items||[];audioMap=new Map(audios.map(x=>[String(x.id),x]));renderCassetteShelf();}catch(e){const root=$('#p9d3CassetteSpines');if(root)root.innerHTML=`<div class="p9d3-audio-error">AUDIO UNAVAILABLE // ${esc(e.message)}</div>`;}}
function renderCassetteShelf(){const root=$('#p9d3CassetteSpines');if(!root)return;root.replaceChildren();if(!audios.length){root.innerHTML='<div class="p9d3-home-empty">NO TASK AUDIO // RECORD INSIDE A TASK</div>';return;}for(const a of audios){const pref=cassettePrefs(a.id),b=document.createElement('button');b.className='p9d3-cassette-spine';b.type='button';b.dataset.audio=a.id;b.style.setProperty('--shell',pref.shell||'#681b2b');b.style.setProperty('--accent',pref.accent||'#ff315c');b.innerHTML=`<span>${esc((pref.label||a.label||a.taskTitle||'AUDIO').slice(0,24))}</span><i></i>`;b.onclick=()=>openCassette(a,b);root.append(b);}}
async function fetchAudioBlob(a){try{const r=await fetch(`${API_BASE}${a.url}`,{credentials:'include'});if(!r.ok)throw new Error(`AUDIO FILE // HTTP ${r.status}`);return await r.blob();}catch(e){throw new Error(`AUDIO RETRIEVAL FAILED // ${e.message}`)}}
...
async function openCassette(a,spine){
  ...
  try{const blob=await fetchAudioBlob(a);cassette.blob=blob;if(audioBlobUrl)URL.revokeObjectURL(audioBlobUrl);audioBlobUrl=URL.createObjectURL(blob);const p=$('#p9d3Audio');cassette.player=p;p.src=audioBlobUrl;p.volume=cassette.volume;p.onloadedmetadata=()=>{cassette.duration=Number.isFinite(p.duration)?p.duration:0;$('#p9d3Duration').textContent=formatTime(cassette.duration);cassette.segments=buildSegments(a,cassette.duration);renderTranscriptLines();$('#p9d3AudioState').textContent='READY';};p.ontimeupdate=()=>{if(cassette?.direction===1){cassette.current=p.currentTime;updateCassetteUi();}};p.onplay=()=>{cassette.playing=true;cassette.direction=1;startReelLoop();syncPlayButton();};p.onpause=()=>{if(cassette?.direction===1)cassette.playing=false;syncPlayButton();};p.onended=()=>{cassette.playing=false;syncPlayButton();};}catch(e){$('#p9d3AudioState').textContent=e.message;$('#p9d3AudioState').classList.add('error');toast(e.message,true);}
}
function bindCassetteControls(){
  const stage=$('#p9d3CassetteStage');stage.querySelector('[data-cass="play"]').onclick=toggleCassettePlay;stage.querySelector('[data-cass="replay"]').onclick=()=>seekCassette(0);stage.querySelector('[data-cass="back"]').onclick=()=>seekCassette(cassette.current-10);stage.querySelector('[data-cass="forward"]').onclick=()=>seekCassette(cassette.current+10);stage.querySelector('[data-cass="reverse"]').onclick=toggleReverse;stage.querySelector('[data-cass="transcribe"]').onclick=transcribeOpenCassette;$('#p9d3Seek').oninput=e=>seekCassette((Number(e.target.value)/1000)*(cassette.duration||0),true);$('#p9d3CassSpeed').onchange=e=>{cassette.speed=Number(e.target.value)||1;if(cassette.player)cassette.player.playbackRate=cassette.speed;if(cassette.direction===-1&&cassette.playing)startReversePlayback();};$('#p9d3CassVolume').oninput=e=>{cassette.volume=Number(e.target.value)/100;if(cassette.player)cassette.player.volume=cassette.volume;if(reverseGain)reverseGain.gain.value=cassette.volume;};
  const savePref=async()=>{const p=cassette.pref={...cassette.pref,label:$('#p9d3CassLabel').value.trim()||cassette.item.label,shell:$('#p9d3CassShell').value,accent:$('#p9d3CassAccent').value,style:$('#p9d3CassStyle').value};saveCassettePrefs(cassette.item.id,p);try{const d=await api(`/api/v8/phase9d2/task-audio/${encodeURIComponent(cassette.item.id)}`,{method:'PATCH',body:{label:p.label}});cassette.item={...cassette.item,...d.item};}catch{}renderCassetteShelf();const v=$('#p9d3CassetteStage .p9d3-cassette-visual');if(v)v.innerHTML=cassetteSvg(p.label,p);};$('#p9d3CassLabel').onchange=savePref;$('#p9d3CassShell').oninput=savePref;$('#p9d3CassAccent').oninput=savePref;$('#p9d3CassStyle').onchange=savePref;
}
...
function buildSegments(a,dur){const raw=a.transcriptMeta?.segments;if(Array.isArray(raw)&&raw.length)return raw.map(x=>({start:Number(x.start||x.startSec||0),end:Number(x.end||x.endSec||x.start||0)+.1,text:String(x.text||'').trim()})).filter(x=>x.text);const sentences=String(a.transcript||'').split(/(?<=[.!?])\s+/).filter(Boolean);if(!sentences.length)return[];const span=Math.max(1,dur||sentences.length)/sentences.length;return sentences.map((text,i)=>({start:i*span,end:(i+1)*span,text}));}
...
async function ensureReverseBuffer(){if(reverseBuffer)return reverseBuffer;if(!cassette?.blob)throw new Error('Audio data is not ready.');reverseCtx=reverseCtx||new AudioContext();const arr=await cassette.blob.arrayBuffer(),buf=await reverseCtx.decodeAudioData(arr.slice(0)),rev=reverseCtx.createBuffer(buf.numberOfChannels,buf.length,buf.sampleRate);for(let c=0;c<buf.numberOfChannels;c++){const src=buf.getChannelData(c),dst=rev.getChannelData(c);for(let i=0,j=src.length-1;i<src.length;i++,j--)dst[i]=src[j];}reverseBuffer=rev;return rev;}
