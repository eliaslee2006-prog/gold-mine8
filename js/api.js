...
function parseReminder(value){
  ...
  try{const x=typeof value==='string'?JSON.parse(value):value;if(x&&typeof x==='object')return{useDefault:!!x.useDefault,overrides:Array.isArray(x.overrides)?x.overrides:[]};}catch{}
  return{useDefault:false,overrides:[]};
}
function reminderText(reminder){
  const r=parseReminder(reminder);if(r.useDefault)return'ALERT // GOOGLE DEFAULT';if(!r.overrides.length)return'';
  const sorted=[...r.overrides].sort((a,b)=>Number(a.minutes)-Number(b.minutes));const first=sorted[0],n=Number(first.minutes)||0;
  let t=n<60?`T-${n}M`:n%1440===0?`T-${n/1440}D`:n%60===0?`T-${n/60}H`:`T-${n}M`;
  return`ALERT // ${t}${sorted.length>1?` +${sorted.length-1}`:''}`;
}
function reminderUnit(minutes){minutes=Number(minutes)||0;if(minutes&&minutes%1440===0)return[minutes/1440,'days'];if(minutes&&minutes%60===0)return[minutes/60,'hours'];return[minutes||30,'minutes'];}
...
function addReminderRow(data={method:'popup',minutes:30}){
  const list=$('#reminderList');if(list.children.length>=5){onChanged('Google Calendar allows up to 5 reminder overrides.',true);return}
  const [value,unit]=reminderUnit(data.minutes);const row=document.createElement('div');row.className='reminder-row';
  row.innerHTML=`<select class="reminder-method"><option value="popup" ${data.method==='popup'?'selected':''}>POPUP / MOBILE</option><option value="email" ${data.method==='email'?'selected':''}>EMAIL</option></select><input class="reminder-value" type="number" min="0" max="40320" value="${value}"><select class="reminder-unit"><option value="minutes" ${unit==='minutes'?'selected':''}>MINUTES BEFORE</option><option value="hours" ${unit==='hours'?'selected':''}>HOURS BEFORE</option><option value="days" ${unit==='days'?'selected':''}>DAYS BEFORE</option></select><button type="button" class="reminder-remove" aria-label="Remove reminder">×</button>`;
  row.querySelector('.reminder-remove').onclick=()=>{row.remove();syncReminderUi()};row.querySelectorAll('select,input').forEach(x=>x.oninput=syncReminderUi);list.append(row);syncReminderUi();
}
function reminderFromForm(){
  const mode=$('#reminderMode').value;if(mode==='default')return JSON.stringify({useDefault:true,overrides:[]});if(mode!=='custom')return JSON.stringify({useDefault:false,overrides:[]});
  const overrides=[...document.querySelectorAll('#reminderList .reminder-row')].slice(0,5).map(row=>({method:row.querySelector('.reminder-method').value,minutes:reminderMinutes(row.querySelector('.reminder-value').value,row.querySelector('.reminder-unit').value)}));
  return JSON.stringify({useDefault:false,overrides});
}
function setReminderForm(value){
  const r=parseReminder(value);$('#reminderList').innerHTML='';if(r.useDefault)$('#reminderMode').value='default';else if(r.overrides.length)$('#reminderMode').value='custom';else $('#reminderMode').value='none';
  for(const x of r.overrides.slice(0,5))addReminderRow(x);if($('#reminderMode').value==='custom'&&!r.overrides.length)addReminderRow();syncReminderUi();
}
function syncReminderUi(){
  const mode=$('#reminderMode').value;$('#reminderCustom').classList.toggle('hidden',mode!=='custom');if(mode==='custom'&&!$('#reminderList').children.length)addReminderRow();
  const raw=reminderFromForm(),txt=reminderText(raw);$('#reminderSummary').textContent=mode==='none'?'NO REMINDER':mode==='default'?'GOOGLE DEFAULT REMINDERS':txt||'CUSTOM REMINDER';
}
...
function renderEventBlock(lane,ev,startH,endH){
  ...
  const alert=reminderText(ev.reminder);el.innerHTML=`<div class="event-content"><div class="event-time">${formatTime(s)} — ${formatTime(e)}</div><div class="event-title">${escapeHtml(ev.title)}</div><div class="event-type">${escapeHtml(ev.category||'EVENT')}${ev.repeatRule?' // REPEATING':''}${alert?`<span class="event-alert">${escapeHtml(alert)}</span>`:''}</div></div><div class="event-resize" aria-label="Resize"></div>`;
...
function renderAgenda(){
  ...
  for(const key of Object.keys(by).sort()){const d=new Date(`${key}T00:00:00`),sec=document.createElement('section');sec.className='agenda-day';sec.innerHTML=`<div class="agenda-date"><strong>${d.toLocaleDateString('en-SG',{day:'2-digit',month:'short'}).toUpperCase()}</strong><span>${d.toLocaleDateString('en-SG',{weekday:'long'}).toUpperCase()}</span></div>`;for(const ev of by[key]){const row=document.createElement('div');row.className='agenda-event';row.style.setProperty('--event-color',ev.color||categoryColor(ev.category));const alert=reminderText(ev.reminder);row.innerHTML=`<div class="agenda-time">${ev.allDay?'ALL DAY':formatTime(ev.occurrenceStart)}</div><div class="agenda-bar"></div><div><div class="agenda-title">${escapeHtml(ev.title)}</div><div class="agenda-type">${escapeHtml(ev.category||'EVENT')}${ev.repeatRule?' // REPEATING':''}${alert?` // ${escapeHtml(alert)}`:''}</div></div><span>›</span>`;row.onclick=()=>openEventDialog(ev);sec.append(row)}wrap.append(sec)}
  root.append(wrap);
}
...
export function initCalendar(changed){
  ...
  $('#reminderMode').onchange=syncReminderUi;$('#addReminderBtn').onclick=()=>addReminderRow();
...
function openEventDialog(item=null,start=null,end=null){
  ...
  const range=parent?[new Date(parent.startAt),new Date(parent.endAt)]:[start,end];$('#eventStart').value=localInput(range[0]);$('#eventEnd').value=localInput(range[1]);$('#deleteEventBtn').classList.toggle('hidden',!parent);setRepeatFromRule(parent?.repeatRule,range[0]);setReminderForm(parent?.reminder);$('#eventDialog').showModal();
}
...
async function saveEvent(){
  ...
  const body={title:$('#eventTitle').value.trim(),category:$('#eventCategory').value,color:$('#eventColor').value,startAt:start.toISOString(),endAt:end.toISOString(),location:$('#eventLocation').value.trim(),notes:$('#eventNotes').value.trim(),allDay:false,repeatRule:repeatRuleFromForm(start),reminder:reminderFromForm()};
